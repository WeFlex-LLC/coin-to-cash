...
export class CoinService {
    ...
    async getCoinPairs(coinToTypeId: number): Promise<any[]> {
        return await this.pairRepository
            ...
            .where('pair.toId = :coinToTypeId and pair.isActive = 1', { coinToTypeId })
            .getRawMany();
    }
    ...
    async getInterval(pairId: number, amount: number){
        return await this.intervalRepository
            ...
            .where('interval.pairId = :pairId and ( :amount >= interval.from and :amount <= interval.to ) or ( :amount >= interval.from and interval.to is null ) or ( interval.to is null and interval.from is null )')
            .setParameters({
                pairId,
                amount
            })
