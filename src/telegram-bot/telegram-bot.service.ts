...
export class TelegramBotService {
  private pendingOrders: { [id: number]: { pairId: number, option: 0 | 1, amount?: number, placeId?: number } } = { };
  ...
  async start(@Ctx() ctx) {
    ...
    if(!user)
      user = await this.usersService.addUser({ telegramId: id, lang: language_code });
  ...
  async chooseCoin(@Ctx() ctx) {
    ...
    const user = await this.usersService.getUserByTelegramId(username);
  ...
  async writeAmount(@Ctx() ctx) {
    ...
    if(this.pendingOrders[username]){
      this.pendingOrders[username].amount = amount;
      const interval = await this.coinService.getInterval(this.pendingOrders[username].pairId, amount);
  ...
  async changeLang(@Ctx() ctx) {
    let keyboard = [Object.values(Lang).map((e)=>({ text: e, callback_data: 'selectLang(' + e + ')' }))]
  ...
  async buyCoin(@Ctx() ctx) {
    ...
    const user = await this.usersService.getUserByTelegramId(ctx.update.callback_query.from.id);
    ...
    let keyboard = chunk(coinToPairsFrom.map((e)=>({ text: e['name_' + user.lang], callback_data: 'selectOption(' + e.pairId + ')' })), 2);
  ...
  async selectOption(@Ctx() ctx) {
    ...
    let keyboard = [
      [{ text: `Give ${pair.from['name_' + user.lang]}`, callback_data: `registerPair(${pairId},0)` }],
      [{ text: `Take ${pair.to['name_' + user.lang]}`, callback_data: `registerPair(${pairId},1)` }]
    ];
  ...
  async registerPair(@Ctx() ctx) {
    const pairId = ctx.match[1];
    const option = +ctx.match[2] as 0 | 1; // 0 - give, 1 - take
    const pair = await this.coinService.getPairInfo(pairId);
    const user = await this.usersService.getUserByTelegramId(ctx.update.callback_query.from.id);

    if (!this.pendingOrders[user.telegramId]) this.addTimeout(user.telegramId.toString(), 1000 * 60 * 10);

    this.pendingOrders[user.telegramId] = { pairId: pairId, option };

    if(pair.from.type == CoinType.Cash || pair.to.type == CoinType.Cash){
      const places = await this.coinService.getPlaces(option ? pair.toId : pair.fromId);

      let keyboard = chunk(places.map((e)=>({ text: e['name_' + user.lang], callback_data: 'selectPlace(' + e.id + ')' })), 2);
      ...
      const coinToType = await this.coinService.getCoinToTypeInfo(this.pendingOrders[user.telegramId].option ? pair.toId : pair.fromId);
  
      await ctx.reply(`Write the amount you want to ${this.pendingOrders[user.telegramId].option ? 'take' : 'give'} ${coinToType['name_' + user.lang]}`);
    }
  }
  ...
  async selectPlace(@Ctx() ctx) {
    ...
    const user = await this.usersService.getUserByTelegramId(ctx.update.callback_query.from.id);
    const pair = await this.coinService.getPairInfo(this.pendingOrders[user.telegramId].pairId);
    const coinToType = await this.coinService.getCoinToTypeInfo(this.pendingOrders[user.telegramId].option ? pair.toId : pair.fromId);
  ...
  async showWhatYouWant(@Ctx() ctx, user: User){
    ...
    let keyboard = chunk(coinToTypes.map((e)=>({ text: e['name_' + user.lang], callback_data: 'buyCoin(' + e.id + ')' })), 2);
